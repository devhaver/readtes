 ...
 */
import type { Toc, TocChapter, TocPart, TocVolume } from '~~/shared/types/content'
...
export const findChapter = (toc: Toc, chapterId: string): FlattenedChapter | undefined =>
...
export const breadcrumbFor = (toc: Toc, chapterId: string): Breadcrumb | null => {
  const entry = findChapter(toc, chapterId)
  return entry ? { volume: entry.volume, part: entry.part, chapter: entry.chapter } : null
}
