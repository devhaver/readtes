...
737 chapters where the Hebrew and English align — is 307 KB, three quarters of
it citation excerpts. `/glossary` loads a derived ~35 KB index up front and
fetches the excerpts only when a reader opens a term. Neither chunk is
prefetchable, so no other page pays for it.
