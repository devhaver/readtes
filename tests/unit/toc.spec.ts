...
describe('flattenChapters', () => {
  it('flattens every chapter in volume -> part -> chapter reading order', () => {
    expect(flattenChapters(fixtureToc).map(entry => entry.chapter.id)).toEqual([
      'part-01/chapter-01',
      'part-01/chapter-02',
      'part-01/chapter-03',
      'part-02/chapter-01',
...
describe('prevNextChapter', () => {
  ...
  it('returns nulls for an unknown chapter id', () => {
    expect(prevNextChapter(fixtureToc, 'nope')).toEqual({ prev: null, next: null })
  })
})
