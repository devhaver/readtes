...
describe("sanitizeHtml — allowlist behavior", () => {
  it("keeps allowed inline tags", () => {
    const html =
      "<p><b>bold</b> <i>italic</i> <em>em</em> <strong>strong</strong> <small>small</small> <big>big</big></p>";
  ...
  it("removes script/style tags along with their content", () => {
    ...
    expect(sanitizeHtml("before<style>.x{color:red}</style>after")).toBe(
      "beforeafter",
    );
  });
});
...
describe("sanitizeHtml — attribute stripping", () => {
  it("strips onclick and style attributes from allowed tags", () => {
    const html =
      '<span onclick="alert(1)" style="color:red" class="ok">text</span>';
  ...
  it("keeps only the allowlisted attributes for <a>", () => {
    const html =
      '<a href="#op-1" class="tes-anchor" data-anchor="op-1" id="x" target="_blank">א</a>';
  ...
  it("keeps title on span but not on sup", () => {
    expect(sanitizeHtml('<span title="tip">t</span>')).toBe(
      '<span title="tip">t</span>',
    );
    expect(sanitizeHtml('<sup title="tip" class="x">1</sup>')).toBe(
      '<sup class="x">1</sup>',
    );
  });
});
...
describe("sanitizeHtml — footnote conversion", () => {
  it("converts a footnote-marker/footnote pair into a tes-footnote span", () => {
    const html =
      '<sup class="footnote-marker">*</sup><i class="footnote">This is the footnote text.</i>';
  ...
  it("strips nested markup inside the footnote text before using it as a title", () => {
    const html =
      '<sup class="footnote-marker">*</sup><i class="footnote">See <b>Zohar</b>, part 1.</i>';
  ...
  it("converts a footnote pair embedded within surrounding text", () => {
    const html =
      'Text before<sup class="footnote-marker">1</sup><i class="footnote">Note.</i>text after';
  ...
  it("escapes ampersands and quotes in footnote text exactly once", () => {
    const html =
      '<sup class="footnote-marker">*</sup><i class="footnote">He said "Kavod" & left.</i>';
  ...
  it("escapes angle brackets in footnote text exactly once", () => {
    const html =
      '<sup class="footnote-marker">*</sup><i class="footnote">5 > 3 and 2 < 4</i>';
  ...
  it("does not re-escape the synthetic footnote span when other tags follow it", () => {
    const html =
      '<sup class="footnote-marker">*</sup><i class="footnote">A & B</i><a href="#op-1" class="tes-anchor">link</a>';
