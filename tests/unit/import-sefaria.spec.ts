...
import { buildChapterUnits, chapterSlug } from '../../scripts/lib/chapter-units.ts'
...
import { alignJaggedArrays, normalizeToChapterItemLists } from '../../scripts/lib/jagged-array.ts'
import { chapterRefFor, ohrPenimiChapterRef, ohrPenimiItemRef, segmentRefFor } from '../../scripts/lib/sefaria-refs.ts'
import { findMainTextNode, findSectionNode, findSiblingNodes, mapNodeTitleToKind } from '../../scripts/lib/sefaria-index.ts'
import { buildTocChapter, mainChapterTitle, siblingChapterTitle, sortTocChapters } from '../../scripts/lib/toc-builder.ts'
import { buildCommentaryItems, buildSourceSegments, parseCommentaryLinks } from '../../scripts/lib/transform.ts'
import type { SefariaIndex, SefariaLink, SefariaV3TextsResponse } from '../../scripts/lib/sefaria-api-types.ts'

const fixturesDir = join(process.cwd(), 'tests/fixtures/sefaria-import')
const readFixture = <T>(name: string): T => JSON.parse(readFileSync(join(fixturesDir, name), 'utf-8')) as T
...
const ohrPenimiText = readFixture<SefariaV3TextsResponse>('ohr-penimi-text.json')
...
describe('sefaria-index: node resolution', () => {
  ...
  it('returns undefined for an unknown sefariaNode', () => {
    expect(findSectionNode(index, 'Talmud Eser HaSefirot, Section XCIX')).toBeUndefined()
  })

  it('finds the main-text ("default") node', () => {
    const section = findSectionNode(index, 'Talmud Eser HaSefirot, Section I')!
    const mainNode = findMainTextNode(section)
    expect(mainNode).toMatchObject({ key: 'default', depth: 2, sectionNames: ['Chapter', 'Seif'] })
  })
  ...
  it('maps known sibling titles to a ChapterKind', () => {
    ...
    expect(mapNodeTitleToKind('List of Questions on Terminology')).toBe('questions-terminology')
    expect(mapNodeTitleToKind('List of Questions on Topics')).toBe('questions-topics')
    expect(mapNodeTitleToKind('List of Answers on Terminology')).toBe('answers-terminology')
    expect(mapNodeTitleToKind('List of Answers on Topics')).toBe('answers-topics')
  })

  it('returns undefined for an unmapped sibling title (e.g. Section VI\'s Cause and Effect lists) — never guesses', () => {
    const section = findSectionNode(index, 'Talmud Eser HaSefirot, Section VI')!
    const siblings = findSiblingNodes(section)
    expect(siblings.map(n => n.title)).toContain('List of Questions on Cause and Effect')
    expect(mapNodeTitleToKind('List of Questions on Cause and Effect')).toBeUndefined()
  })
})

describe('jagged-array: normalizeToChapterItemLists', () => {
  it('passes depth-2 nested arrays through unchanged', () => {
    const shaped = normalizeToChapterItemLists({ depth: 2 }, [['a', 'b'], ['c']])
  ...
  it('wraps each item as its own chapter for depth-1 "Chapter"-addressed nodes (Sections V+ main text)', () => {
    const shaped = normalizeToChapterItemLists({ depth: 1, sectionNames: ['Chapter'] }, ['whole chapter 1', 'whole chapter 2'])
  ...
  it('treats a depth-1 non-"Chapter" node as one implicit chapter containing every item', () => {
    const shaped = normalizeToChapterItemLists({ depth: 1, sectionNames: ['Paragraph'] }, ['q1', 'q2', 'q3'])
...
describe('sefaria-refs', () => {
  it('builds a chapter ref by appending the chapter index', () => {
    expect(chapterRefFor('Talmud Eser HaSefirot, Section I', 2)).toBe('Talmud Eser HaSefirot, Section I 2')
  })

  it('a chapter ref with no chapterIndex is the base ref itself (single implicit chapter)', () => {
    expect(chapterRefFor('Talmud Eser HaSefirot, Section I, List of Questions on Terminology', undefined)).toBe(
      'Talmud Eser HaSefirot, Section I, List of Questions on Terminology',
    )
  })

  it('builds a depth-2 segment ref with a colon separator', () => {
    const ref = segmentRefFor('Talmud Eser HaSefirot, Section I 1', { depth: 2, sectionNames: ['Chapter', 'Seif'] }, 3)
  ...
  it('a depth-1 "Chapter" segment ref is the chapter ref itself (whole chapter is the one segment)', () => {
    const ref = segmentRefFor('Talmud Eser HaSefirot, Section V 3', { depth: 1, sectionNames: ['Chapter'] }, 1)
  ...
  it('a depth-1 non-"Chapter" segment ref appends the item index with a space', () => {
    ...
    expect(ref).toBe('Talmud Eser HaSefirot, Section I, List of Questions on Terminology 5')
  })

  it('builds Ohr Penimi chapter/item refs', () => {
    const chapterRef = ohrPenimiChapterRef('Ohr Penimi on Talmud Eser HaSefirot', 1, 1)
    expect(chapterRef).toBe('Ohr Penimi on Talmud Eser HaSefirot 1:1')
    expect(ohrPenimiItemRef(chapterRef, 3)).toBe('Ohr Penimi on Talmud Eser HaSefirot 1:1:3')
  })
})

describe('heading: extractLeadingHeading', () => {
  it('peels a chained <b><big>…</big></b> + <small>…</small> leading gloss', () => {
    const html = '<b><big>Topic line</big></b><br><br><small>Short gloss</small><br>Body text follows.'
    expect(extractLeadingHeading(html)).toEqual({ heading: 'Topic line — Short gloss', rest: 'Body text follows.' })
  })

  it('peels a single leading <small> gloss', () => {
    const html = '<small>Just a gloss.</small><br>Body text.'
    expect(extractLeadingHeading(html)).toEqual({ heading: 'Just a gloss.', rest: 'Body text.' })
  })
...
describe('chapter-units: buildChapterUnits (against the real depth-2 main-text fixture)', () => {
  ...
  it('builds stable chapterId/chapterRef per unit', () => {
    expect(units[0]).toMatchObject({ chapterId: 'part-01/chapter-01', chapterRef: 'Talmud Eser HaSefirot, Section I 1' })
    expect(units[1]).toMatchObject({ chapterId: 'part-01/chapter-02', chapterRef: 'Talmud Eser HaSefirot, Section I 2' })
  })
...
describe('chapter-units: sibling node shapes', () => {
  it('a depth-1 Paragraph sibling node (Questions list) resolves to exactly one chapter unit', () => {
    ...
    const units = buildChapterUnits('part-01', 'questions-terminology', node, 'Talmud Eser HaSefirot, Section I, List of Questions on Terminology', ['q1', 'q2', 'q3'], undefined)
    expect(units).toHaveLength(1)
    expect(units[0]).toMatchObject({
      number: 1,
      chapterId: 'part-01/questions-terminology-01',
      chapterRef: 'Talmud Eser HaSefirot, Section I, List of Questions on Terminology',
      heItems: ['q1', 'q2', 'q3'],
      enItems: [],
    })
  })
  ...
  it('a depth-2 sibling node (Histaklut Penimit / Answers) resolves to one chapter unit per outer index', () => {
    ...
    const units = buildChapterUnits('part-01', 'inner-observation', node, 'Talmud Eser HaSefirot, Section I, Histaklut Penimit', [['a', 'b'], ['c']], undefined)
    expect(units.map(u => u.chapterId)).toEqual(['part-01/inner-observation-01', 'part-01/inner-observation-02'])
    expect(units[0]?.chapterRef).toBe('Talmud Eser HaSefirot, Section I, Histaklut Penimit 1')
  })
})
...
describe('transform: buildSourceSegments', () => {
  ...
  it('extracts a chained heading, converts anchors and a footnote, in one pass (chapter 1 seif 1, Hebrew)', () => {
    const { segments, droppedAnchors } = buildSourceSegments(mainNode, 'Talmud Eser HaSefirot, Section I 1', versionText(mainText, 'he')[0] as string[], true)
  ...
  it('extracts a single-<small> heading on a later seif (chapter 1 seif 2)', () => {
    const { segments } = buildSourceSegments(mainNode, 'Talmud Eser HaSefirot, Section I 1', versionText(mainText, 'he')[0] as string[], true)
  ...
  it('drops a non-Ohr-Penimi inline marker from anchors[] and neutralizes it to bare label text in html (chapter 2 seif 1)', () => {
    const { segments, droppedAnchors } = buildSourceSegments(mainNode, 'Talmud Eser HaSefirot, Section I 2', versionText(mainText, 'he')[1] as string[], true)
    ...
    expect(droppedAnchors).toEqual([{ sefariaRef: 'Talmud Eser HaSefirot, Section I 2:1', commentator: 'Histaklut Pnimit', order: 99 }])
  })

  it('skips "" positions (no text in this language) rather than emitting an empty segment', () => {
    const { segments } = buildSourceSegments(mainNode, 'Talmud Eser HaSefirot, Section I 1', ['only seif one', ''], true)
    expect(segments).toHaveLength(1)
    expect(segments[0]?.n).toBe(1)
  })

  it('does not extract a heading when extractHeadings is false (sibling-node scoping)', () => {
    const { segments } = buildSourceSegments({ depth: 1, sectionNames: ['Paragraph'] }, 'ref base', ['<small>looks like a heading</small><br>body'], false)
...
describe('transform: parseCommentaryLinks', () => {
  const { byChapter, warnings } = parseCommentaryLinks(links, 'Ohr Penimi on Talmud Eser HaSefirot')
  ...
  it('groups Ohr Penimi commentary links by the chapter parsed from anchorRef, ignoring other categories/indices', () => {
    ...
    expect(byChapter.get(2)).toEqual([{ order: 1, targetSeif: 1, heLabel: 'א' }])
  })
...
describe('transform: buildCommentaryItems', () => {
  const heChapters = normalizeToChapterItemLists({ depth: 2 }, versionText(ohrPenimiText, 'he'))
  const enChapters = alignJaggedArrays(heChapters, normalizeToChapterItemLists({ depth: 2 }, versionText(ohrPenimiText, 'en')))
  const chapterRef = ohrPenimiChapterRef('Ohr Penimi on Talmud Eser HaSefirot', 1, 1)
  const chapterLinks = [
    { order: 1, targetSeif: 1, heLabel: 'א' },
    { order: 2, targetSeif: 1, heLabel: 'ב' },
  ...
  it('builds Hebrew commentary items with bilingual labels (en label = order number)', () => {
    const { items, warnings } = buildCommentaryItems(chapterRef, heChapters[0] as string[], chapterLinks, 'he')
    expect(warnings).toEqual([])
  ...
  it('strips the leading "N." from English text and cross-checks it against data-order, warning on mismatch but trusting the ref', () => {
    const { items, warnings } = buildCommentaryItems(chapterRef, enChapters[0] as string[], chapterLinks, 'en')

    expect(items).toHaveLength(2) // order 3's en text is "" — skipped
    expect(items[0]).toMatchObject({ anchorId: 'op-1', order: 1, html: 'Simple light - meaning it has no change of form.' })
    expect(items[1]).toMatchObject({ anchorId: 'op-2', order: 2, html: 'Wrong leading number on purpose, to test the mismatch-warning path.' })
    expect(warnings).toEqual([
  ...
  it('skips an item that has text but no matching links entry (no targetSeif available)', () => {
    const { items, warnings } = buildCommentaryItems(chapterRef, ['orphan text, no link for this order'], [], 'he')
    expect(items).toEqual([])
    expect(warnings).toEqual([`${chapterRef}: commentary item order 1 (he) has text but no links entry for it — skipped`])
  })
})
...
describe('toc-builder', () => {
  ...
  it('siblingChapterTitle uses the node title verbatim when it is the only instance', () => {
    const title = siblingChapterTitle({ title: 'List of Questions on Terminology', heTitle: 'לוח השאלות לפירוש המלות' }, 1, 1)
    expect(title).toEqual({ en: 'List of Questions on Terminology', he: 'לוח השאלות לפירוש המלות' })
  })

  it('siblingChapterTitle numbers the node title when there are several instances', () => {
    const title = siblingChapterTitle({ title: 'Histaklut Penimit', heTitle: 'הסתכלות פנימית' }, 3, 10)
    expect(title).toEqual({ en: 'Histaklut Penimit 3', he: 'הסתכלות פנימית ג׳' })
  })

  it('buildTocChapter derives availableLayers/availableVersions from files on disk', () => {
    const chapter = buildTocChapter('part-01/chapter-01', 'chapter', 1, { en: 'Chapter 1', he: 'פרק א' }, {
      summary: ['en-curated'],
  ...
  it('sortTocChapters orders by kind (chapter first) then by number', () => {
    const chapters = [
      buildTocChapter('part-01/questions-terminology-01', 'questions-terminology', 1, { en: 'Q' }, { summary: [], source: ['he'], commentary: [] }),
      buildTocChapter('part-01/chapter-02', 'chapter', 2, { en: 'Chapter 2' }, { summary: [], source: ['he'], commentary: [] }),
      buildTocChapter('part-01/chapter-01', 'chapter', 1, { en: 'Chapter 1' }, { summary: [], source: ['he'], commentary: [] }),
    ]
...
describe('coverage-report: buildCoverageMarkdown', () => {
    ...
    const markdown = buildCoverageMarkdown([
      {
        ...
        partTitle: 'Section I',
        stats: [
          { layer: 'source', versionId: 'he-jerusalem-1956', chaptersTotal: 2, chaptersWithText: 2, segments: 10, emptyChapterIds: [] },
          { layer: 'source', versionId: 'en-sefaria-community', chaptersTotal: 2, chaptersWithText: 1, segments: 5, emptyChapterIds: ['part-01/chapter-02'] },
        ],
        warnings: ['section "Talmud Eser HaSefirot, Section I": unknown sibling node "Foo" — no ChapterKind mapping, skipped entirely'],
      },
    ])
  ...
  it('is a pure function of its input (calling it twice with the same input is byte-identical)', () => {
    const input = [{ partId: 'part-01', partTitle: 'Section I', stats: [], warnings: [] }]
