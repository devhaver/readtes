...
export const mapNodeTitleToKind = (nodeTitle: string): ChapterKind | undefined => KIND_BY_NODE_TITLE[nodeTitle]
 ...
 */
export const findSectionNode = (index: SefariaIndex, sefariaNode: string): SefariaIndexNode | undefined => {
  const prefix = `${index.title}, `
  const sectionTitle = sefariaNode.startsWith(prefix) ? sefariaNode.slice(prefix.length) : sefariaNode
  return index.schema.nodes.find(node => node.title === sectionTitle)
}

/** The section node's main-text node — the child keyed `"default"`. */
export const findMainTextNode = (sectionNode: SefariaIndexNode): SefariaIndexNode | undefined =>
...
export const findSiblingNodes = (sectionNode: SefariaIndexNode): SefariaIndexNode[] =>
